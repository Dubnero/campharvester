...
export function PublicDirectory({ initialCamps, initialProviders }: Props) {
  ...
  const sortedCamps = useMemo(
    () => sortPublicCamps(filteredCamps, sort),
    [filteredCamps, sort],
  );
  const counties = useMemo(
    () => getUniquePublicValues(publicCamps, "county"),
    [publicCamps],
  );
  const towns = useMemo(
    () => getPublicTownOptions(publicCamps, filters.county),
    [publicCamps, filters.county],
  );
  const activities = useMemo(
    () => getUniquePublicValues(publicCamps, "activity_type"),
    [publicCamps],
  );
  const holidays = useMemo(
    () => getUniquePublicValues(publicCamps, "holiday_type"),
    [publicCamps],
  );
  const activeChips = useMemo<FilterChip[]>(() => {
    ...
    if (filters.activity)
      chips.push({ key: "activity", label: filters.activity });
    if (filters.startDate)
      chips.push({ key: "startDate", label: `From ${filters.startDate}` });
    if (filters.endDate)
      chips.push({ key: "endDate", label: `To ${filters.endDate}` });
    if (filters.dayLength)
      chips.push({
        key: "dayLength",
        label: publicDayLengthLabel(filters.dayLength),
      });
    if (filters.priceStatus)
      chips.push({
        key: "priceStatus",
        label:
          filters.priceStatus === "present" ? "Price shown" : "Price missing",
      });
    if (filters.verifiedOnly)
      chips.push({ key: "verifiedOnly", label: "Verified only" });
    if (filters.featuredOnly)
      chips.push({ key: "featuredOnly", label: "Featured only" });
    return chips;
  }, [filters]);
    ...
    setFilters((current) => {
      if (key === "county" && typeof value === "string") {
        return {
          ...current,
          county: value,
          town: townForCountyOrBlank(publicCamps, value, current.town),
        };
      }
  ...
  useEffect(() => {
    setFilters((current) => {
      const validTown = townForCountyOrBlank(
        publicCamps,
        current.county,
        current.town,
      );
      return validTown === current.town
        ? current
        : { ...current, town: validTown };
    });
  }, [publicCamps]);
  ...
  return (
    <main className="public-shell">
      <section className="public-hero">
        ...
        <p>
          Search by camp, provider, town or county and compare activities, ages,
          dates and prices.
        </p>
      </section>
      ...
      <section className="public-panel" aria-label="Search and filters">
        <div className="public-filters">
          <label className="public-search">
            Search camps
            <input
              placeholder="Camp, provider, town or county"
          ...
          <label>
            ...
            <select
              ...
              onChange={(event) => updateFilter("holiday", event.target.value)}
            >
          ...
          <label>
            Half-day / full-day
            <select
              value={filters.dayLength}
              onChange={(event) =>
                updateFilter("dayLength", event.target.value)
              }
            >
              ...
              <option>Both</option>
              <option value="Unknown">Length to be confirmed</option>
            </select>
          </label>
          <label>
            Price
            <select
              ...
              onChange={(event) =>
                updateFilter("priceStatus", event.target.value)
              }
            >
              <option value="">Any price status</option>
              <option value="present">Price present</option>
          ...
          <label className="checkbox-row public-checkbox">
            <input
              ...
              onChange={(event) =>
                updateFilter("featuredOnly", event.target.checked)
              }
            />{" "}
            Featured only
          </label>
        ...
        <div className="directory-heading">
          <div>
            ...
            <p className="trust-note">
              ...
              and pricing with the provider before booking.
            </p>
          </div>
          <label className="sort-control">
            ...
            <select
              value={sort}
              onChange={(event) => setSort(event.target.value as PublicSort)}
            >
              <option value="start-date">Start date earliest first</option>
              <option value="price-asc">Price low to high</option>
              <option value="town-az">Town A-Z</option>
              <option value="provider-az">Provider A-Z</option>
            </select>
          </label>
        </div>
                ...
                <article className="camp-card" key={camp.camp_id}>
                  <div className="camp-card-main">
                    <div className="card-badges">
                      ...
                      <span className="badge muted">{camp.holiday_type}</span>
                    </div>
                    ...
                    {locationLines.primary ? (
                      <p className="location-line">{locationLines.primary}</p>
                    ) : null}
                    {locationLines.secondary ? (
                      <p className="address-line">{locationLines.secondary}</p>
                    ) : null}
                    <div className="flags">
                      {camp.verified || camp.provider?.verified ? (
                        <span className="flag verified">Verified</span>
                      ) : null}
                      {camp.featured || camp.provider?.featured ? (
                  ...
                  <dl className="camp-card-meta">
                    <div>
                      <dt>Dates</dt>
                      <dd>{formatDateRange(camp.start_date, camp.end_date)}</dd>
                    </div>
                    <div>
                      <dt>Times</dt>
                      <dd>
                    ...
                    <div>
                      <dt>Ages</dt>
                      <dd>{formatAgeRange(camp.age_min, camp.age_max)}</dd>
                    </div>
                    <div>
                      <dt>Price</dt>
                      <dd>{camp.price || "Price to be confirmed"}</dd>
                    </div>
                  </dl>
                  {camp.booking_url ? (
                    <a
