...
export function importButtonLabel(selectedCamps: DiscoveryCamp[]) {
  ...
  if (existingCount > 0) return "Import / update selected camps";
  return "Import selected drafts";
}
...
export function DiscoveryAssistant() {
  ...
  const [additionalDetailUrls, setAdditionalDetailUrls] = useState("");
  ...
  const selectedCamps = useMemo(() => campsWithSelection.filter((camp) => selectedCampKeys.has(campStableKey(camp))), [campsWithSelection, selectedCampKeys]);
  const selectedCampImportLabel = importButtonLabel(selectedCamps);
  ...
  async function analyse(event: FormEvent) {
    ...
    if (!manualMode) {
      const response = await fetch("/api/discovery/fetch", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ url: form.sourceUrl, additionalDetailUrls }) });
  ...
  return <main className="app-shell discovery-assistant"><header className="hero"><div><p className="eyebrow">Internal admin · draft discovery</p><h1>Discovery Assistant</h1><p>Analyse provider and camp pages, review deterministic draft records, then import selected unverified drafts.</p></div><div className="hero-actions"><Link className="button-link" href="/">Dashboard</Link></div></header>
    ...
    <ProviderCards providers={providers} update={updateProvider} remove={(index) => setProviders((rows) => rows.filter((_, rowIndex) => rowIndex !== index))} />
    ...
    <section className="panel"><div className="form-actions"><button type="button" className="secondary" onClick={downloadCsv} disabled={!providers.length && !camps.length}>Export CSV</button><button type="button" onClick={importSelected} disabled={!selectedProviders.length && !selectedCamps.length}>{selectedCampImportLabel}</button></div>{importSummary ? <p>{importSummary}</p> : null}</section></main>;
}
...
function AnalysisLogPanel({ log, pages, activeManualUrl, manualDraft, onPaste, onDraftChange, onSubmitManual }: { log: AnalysisLog | null; pages: DiscoveryPageAnalysis[]; activeManualUrl: string; manualDraft: string; onPaste: (url: string) => void; onDraftChange: (value: string) => void; onSubmitManual: () => void }) {
  ...
  const pasteCandidates = pages.filter((page) => page.status !== "extracted" && (page.status === "failed" || page.dynamicWarning || page.extractionBlocked));
  return <>
    <section className="panel"><h2>URL status</h2>{log ? <p className="empty-state">{log.discoveredUrls.length} discovered URL(s). Failed or blocked pages can be extended with manual paste below.</p> : null}{log?.starcamp ? <div className="warning-box compact"><strong>Starcamp listing-to-product crawl</strong><small>Listing pages crawled: {log.starcamp.listingPagesCrawled}</small><small>Pagination pages discovered: {log.starcamp.paginationPagesDiscovered}</small><small>Product URLs discovered: {log.starcamp.productUrlsDiscovered}</small><small>Product URLs crawled: {log.starcamp.productUrlsCrawled}</small><small>Product URLs skipped: {log.starcamp.productUrlsSkipped}</small></div> : null}<div className="summary-list"><ul>{pages.map((page) => <li key={page.url}><strong>{page.status === "failed" ? "Failed" : page.extractionBlocked ? "Blocked" : page.status === "extracted" ? "Manual paste added" : "Analysed"}:</strong> {page.url} <span>{page.failureReason ? `(${page.failureReason})` : page.extractionBlocked ? "(analysed as Starcamp listing index)" : page.dynamicWarning ? "(possible JavaScript-rendered content)" : ""}</span>{page.status === "extracted" ? <><br />✓ Page extracted</> : null}</li>)}</ul></div></section>
    {pasteCandidates.length || activeManualUrl ? <section className="panel manual-paste-panel"><h2>Manual paste fallback</h2>{pasteCandidates.length ? <div className="summary-list"><p>Choose a failed or blocked URL, paste its page text, then extract it without changing the URL status list.</p><ul>{pasteCandidates.map((page) => <li key={page.url}><strong>{page.status === "failed" ? "Failed" : "Blocked"} source:</strong> {page.url}<div className="form-actions"><button type="button" className="secondary" onClick={() => onPaste(page.url)}>Paste page text</button></div></li>)}</ul></div> : null}{activeManualUrl ? <div className="manual-paste-box"><h3>Paste page text</h3><p><strong>Source URL:</strong> {activeManualUrl}</p><textarea rows={12} value={manualDraft} onChange={(event) => onDraftChange(event.target.value)} /><div className="form-actions"><button type="button" onClick={onSubmitManual} disabled={!manualDraft.trim()}>Extract pasted page</button></div></div> : null}</section> : null}
  </>;
}
...
const providerTextFields: Array<keyof DiscoveryProvider> = ["confidence", "provider_id", "provider_name", "website", "source_url", "primary_email", "primary_phone", "primary_county", "activity_category", "provider_type", "status", "notes"];
const fullWidthProviderFields = new Set<keyof DiscoveryProvider>(["provider_name", "website", "source_url", "notes"]);

function ProviderCards({ providers, update, remove }: { providers: DiscoveryProvider[]; update: (index: number, field: keyof DiscoveryProvider, value: string | boolean | number) => void; remove: (index: number) => void }) {
  return <section className="panel"><h2>Providers</h2>{providers.length === 0 ? <p className="empty-state">No provider rows found yet.</p> : <div className="provider-review-grid">{providers.map((provider, index) => <article className="provider-review-card" key={`${provider.provider_id}-${index}`}><div className="provider-card-header"><div><span className="badge">{methodBadge(provider.source_method)}</span><ConfidenceBadge score={provider.confidence} /></div><button type="button" className="secondary" onClick={() => remove(index)}>Delete</button></div><h3>{provider.provider_name || provider.provider_id || "Provider draft"}</h3><div className="provider-card-flags"><label className="checkbox-row">Selected<input type="checkbox" checked={provider.selected} onChange={(event) => update(index, "selected", event.target.checked)} /></label><label className="checkbox-row">Needs review<input type="checkbox" checked={provider.needs_review} onChange={(event) => update(index, "needs_review", event.target.checked)} /></label><label className="checkbox-row">Verified<input type="checkbox" checked={provider.verified} onChange={(event) => update(index, "verified", event.target.checked)} /></label><label className="checkbox-row">Featured<input type="checkbox" checked={provider.featured} onChange={(event) => update(index, "featured", event.target.checked)} /></label></div><div className="provider-card-fields"><div className="provider-field-readonly"><span>Source</span><strong>{methodBadge(provider.source_method)}</strong></div>{providerTextFields.map((field) => <label className={fullWidthProviderFields.has(field) ? "provider-field-wide" : ""} key={String(field)}>{label(String(field))}{field === "notes" ? <textarea rows={3} value={String(provider[field] ?? "")} onChange={(event) => update(index, field, event.target.value)} /> : <input value={String(provider[field] ?? "")} onChange={(event) => update(index, field, field === "confidence" ? Number(event.target.value) : event.target.value)} />}</label>)}</div>{provider.duplicateWarnings.length ? <div className="warning-box compact"><strong>Duplicate warnings</strong>{provider.duplicateWarnings.map((warning) => <small key={warning}>⚠ {warning}</small>)}</div> : <div className="success-box compact"><strong>Duplicate warnings</strong><small>No duplicate warnings.</small></div>}</article>)}</div>}</section>;
}
