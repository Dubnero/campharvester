...
type DebugExportPayload = { sourceUrl: string; generatedAt: string; crawlMetrics: AnalysisLog | null; urlStatus: DiscoveryPageAnalysis[]; pagesCrawled: string[]; productUrls: { discovered: string[]; crawled: string[]; skipped: Array<{ url: string; reason: string }> }; debugItems: ExtractionPipelineDebug[] };
...
function buildDebugExportPayload(sourceUrl: string, analysisLog: AnalysisLog | null, pages: DiscoveryPageAnalysis[], items: ExtractionPipelineDebug[]): DebugExportPayload {
  const skipped = analysisLog?.skippedUrls ?? [];
  return { sourceUrl, generatedAt: new Date().toISOString(), crawlMetrics: analysisLog, urlStatus: pages, pagesCrawled: pages.filter((page) => page.status === "analysed").map((page) => page.url), productUrls: { discovered: (analysisLog?.discoveredUrls ?? []).filter(isProductCampUrl), crawled: (analysisLog?.crawledUrls ?? []).filter(isProductCampUrl), skipped: skipped.filter((item) => isProductCampUrl(item.url)) }, debugItems: items };
}
function formatDebugTxt(payload: DebugExportPayload) {
  ...
  lines.push("Crawl metrics", JSON.stringify(payload.crawlMetrics, null, 2), "");
  lines.push("Product URLs", `Discovered (${payload.productUrls.discovered.length}):`, ...payload.productUrls.discovered.map((url) => `- ${url}`), `Fetched (${payload.productUrls.crawled.length}):`, ...payload.productUrls.crawled.map((url) => `- ${url}`), `Skipped (${payload.productUrls.skipped.length}):`, ...payload.productUrls.skipped.map((item) => `- ${item.url} (${item.reason})`), "");
...
export function DiscoveryAssistant() {
  ...
  return <main className="app-shell"><header className="hero"><div><p className="eyebrow">Internal admin · draft discovery</p><h1>Discovery Assistant</h1><p>Analyse provider and camp pages, review deterministic draft records, then import selected unverified drafts.</p></div><div className="hero-actions"><Link className="button-link" href="/">Dashboard</Link></div></header>
    ...
    {developerDebug ? <DeveloperExtractionDebug items={debugItems} sourceUrl={form.sourceUrl} analysisLog={analysisLog} pages={pageAnalyses} /> : null}
