...
function juniorEinsteinsHoliday(text: string, fallback?: string): HolidayType { return /easter/i.test(text) ? "Easter" : /halloween/i.test(text) ? "Halloween" : /midterm/i.test(text) ? "February Midterm" : /summer/i.test(text) ? "Summer" : ((fallback || "Other") as HolidayType); }
