...
export async function POST(req: Request) {
  ...
  try {
    ...
    const selected = selectAiReadableText(body.readable_text);
    ...
    if (!response.ok) return NextResponse.json({ error: `AI extraction failed (${response.status}).` }, { status: 502 });
    const json = await response.json() as { choices?: Array<{ message?: { content?: string } }> };
    const content = json.choices?.[0]?.message?.content || "";
    const raw = parseOpenAIJson(content);
    const mapped = mapAiExtraction(raw, body, selected.text);
    return NextResponse.json({ ...mapped, warnings: selected.wasTrimmed ? [...mapped.warnings, "Readable text was trimmed before AI extraction to control cost."] : mapped.warnings });
  } catch (error) {
